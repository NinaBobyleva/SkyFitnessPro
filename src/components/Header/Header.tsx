...
export default function Header() {
  ...
  const handleUpdateDisplayName = async (newName: string) => {
    await updateDisplayName(newName, setDisplayName);
  }
  ...
  useEffect(() => {
    ...
    const auth = getAuth();
    if (auth.currentUser) {
      onValue(ref(database, `users/${auth.currentUser.uid}/displayName`), (snapshot) => {
        if (snapshot.exists()) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      if (user) {
        ...
        handleUpdateDisplayName(displayName.val() || '');
      } else {
          ...
          <div className="relative">
            ...
            >
              ...
              <div className="flex items-center">
                <img src={user.photoURL || '/img/user-icon.svg'} alt="Аватар пользователя" className="w-10 h-10 rounded-full mr-2" />
