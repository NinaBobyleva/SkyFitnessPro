...
function ProfilePage() {
  ...
  useEffect(() => {
    ...
    return onValue(
      ...
      (snapshot) => {
        if (snapshot.exists()) {
          const userCourseList: UserCourseType[] = Object.entries(
            snapshot.val() as { [key: string]: UserCourseType }
          ).map(([, course]) => ({
            ...course,
          }));
  ...
  return (
    <>
      <Header />
      <Wrapper>
        <div className="box-border md:mb-44 bg-[#FAFAFA]">
        ...
        <div className="flex md:hidden justify-end md:justify-center mb-7 md:w-[140px] mt-8">
          <Link to={"#top"}>
            <Button
